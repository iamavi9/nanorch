...
export default function TasksPage({ orchestratorId, workspaceId }: Props) {
  ...
  const { data: tasks, isLoading } = useQuery<Task[]>({
    queryKey: [`/api/orchestrators/${orchestratorId}/tasks`],
          ...
          return (
            <Card key={status}>
              <CardContent className="p-3 flex items-center gap-2">
                <config.icon className={`w-5 h-5 ${config.color}${config.spin && count > 0 ? " animate-spin" : ""}`} />
                <div>
      ...
      {isLoading ? (
        ...
        <Card className="border-dashed">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-muted-foreground mb-4">Submit your first task to get started</p>
