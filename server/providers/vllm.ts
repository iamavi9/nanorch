...
function getClient(baseUrl: string, apiKey?: string | null) {
  return new OpenAI({
    apiKey: apiKey || "vllm",
...
export async function runVllm(options: RunAgentOptions): Promise<RunAgentResult> {
  if (!options.baseUrl) throw new Error("vLLM requires a base URL (e.g. http://localhost:8000)");

  const client = getClient(options.baseUrl, options.apiKey);
